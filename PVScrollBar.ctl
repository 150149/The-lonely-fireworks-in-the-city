...
Begin VB.UserControl PVScrollBar 
   ...
   ScaleHeight     =   3495
   ScaleWidth      =   255
   ToolboxBitmap   =   "PVScrollBar.ctx":0000
...
Public Property Get Enabled() As Boolean                                        '是否有效
    Enabled = C_Enabled
End Property
...
Private Sub Progress_Click()                                                    '滚动块的单击事件
    If Enabled = True Then RaiseEvent Click                                     '触发单击事件
End Sub

Private Sub Progress_DblClick()                                                 '滚动块的双击事件
    If Enabled = True Then RaiseEvent DblClick                                  '触发双击事件
End Sub
...
Private Sub Progress_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single) '滚动块的鼠标按下事件
    ...
    If Enabled = True Then RaiseEvent MouseDown(Button, Shift, X, Y, C_Value)   '如果有效,触发鼠标按下事件
...
Private Sub Progress_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    End If
    If Enabled = True Then RaiseEvent MouseMove(Button, Shift, X, Y, C_Value)
...
Private Sub Progress_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ClickedY = 0
    If Enabled = True Then RaiseEvent MouseUp(Button, Shift, X, Y, C_Value)
    MouseDowned = False
...
Private Sub UserControl_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    ElseIf KeyCode = -255 Then
        ...
        End If
    End If
    If Enabled Then RaiseEvent Scroll(C_Value)                                  '触发滚动事件
End Sub

Private Sub UserControl_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    If Enabled = True Then RaiseEvent MouseDown(Button, Shift, X, Y, C_Value)
End Sub
