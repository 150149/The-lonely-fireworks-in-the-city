...
Private Sub PButton1_Click()
    ...
    Else
        ...
        read_OK = GetPrivateProfileString(CStr("lock" & Form1.Position), CStr(PListBox1.ListIndex + 1), "0", xg5, 256, App.Path & "\save\save" & Form3.saveid & ".fsave")
        If Form1.moshi = 0 And CInt(xg5) = 0 Then
            Tishim = Tishi("提示", "你没有培训这件事")
            Form3.Tishiback = 8
            Exit Sub
        End If
        
        If CInt(xg1) > Form1.Shejiao Then
            Tishim = Tishi("提示", "你没有足够的社交能力做这件事")
            Form3.Tishiback = 8
            Exit Sub
        End If
        If CInt(xg2) > Form1.Zhili Or CInt(xg3) Then
            Tishim = Tishi("提示", "你没有足够的智力做这件事")
            Form3.Tishiback = 8
            Exit Sub
        End If
        If CInt(xg3) > Form1.Guanchali Then
            Tishim = Tishi("提示", "你没有足够的观察力做这件事")
            Form3.Tishiback = 8
            Exit Sub
        End If
        If CInt(xg4) > Form1.Naili Then
            Tishim = Tishi("提示", "你没有足够的耐力做这件事")
            Form3.Tishiback = 8
            Exit Sub
        End If
        
        
        If Form1.Position = 9 And Form1.Jinrong = False Then
...
Private Sub PButton3_Click()
    ...
    If Replace(first1, Chr(0), "") = "5" And Form1.moshi = 0 Then
        write1 = WritePrivateProfileString("guide", "1", "6", App.Path & "\save\save" & Form3.saveid & ".fsave")
    ElseIf Replace(first1, Chr(0), "") = "3" And Form1.moshi = 2 Then
