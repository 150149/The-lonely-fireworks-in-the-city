...
Begin VB.UserControl PTab 
   ...
   Begin VB.PictureBox P2 
      ...
      Width           =   375
   End
   Begin ÊÐ¾®ÖÐ¹Â°ÁµÄÑÌ»ð.PUIMgr PM 
      Left            =   360
      Top             =   600
      _ExtentX        =   847
      _ExtentY        =   847
...
Private Sub L_Click(Index As Integer)
    If Not Items(Checked).PControl Is Nothing Then Items(Checked).PControl.Visible = False
    l(Checked).ForeColor = C_Color_Text
    Checked = Index
    If Not Items(Checked).PControl Is Nothing Then Items(Checked).PControl.Visible = True
    l(Checked).ForeColor = C_Color_Selected
    PM.MoveSmly p, l(Index).Left - 60, l(Index).Top + l(Index).Height + 45, 1
    PM.SizeSmly p, l(Index).Width + 120, p.Height, 1
...
Private Sub UserControl_Initialize()
    ...
    C_Is_Enabled = True
    C_Distance_Transverse = 240
    C_Distance_Vertical = 45
    C_Color_Selected = &HFFFFFF
    C_Color_Selector = &H0&
    C_Color_Selector_Moved = &H404040
    C_Texts = "PTab"
    C_Height_Selector = 135
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    C_Distance_Transverse = PropBag.ReadProperty("Distance_Transverse", 240)
    C_Distance_Vertical = PropBag.ReadProperty("Distance_Vertical", 45)
    ...
    C_Height_Selector = PropBag.ReadProperty("Height_Selector", 135)
    Reset
End Sub
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("Distance_Transverse", C_Distance_Transverse, 240)
    Call PropBag.WriteProperty("Distance_Vertical", C_Distance_Vertical, 45)
    ...
    Call PropBag.WriteProperty("Height_Selector", C_Height_Selector, 135)
End Sub
